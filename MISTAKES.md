...
- 2026-09-07: `ebay_offer.record_review` forwarded `note=None, verdict=None` to `execute` → XML-RPC cannot marshal None (fb.mark_sold test documents the same trap) → build the kwargs dict from set values only; a routing-mock test that asserts the exact kwargs catches it. (hits: 1)
- 2026-09-07: Modelled `CODED_COUPON` on the markdown fields → `sale_ebay._coupon_body` uses `_order_benefit()` (order_percent/order_amount + optional threshold), not markdown_* → read the server's `_body` builder for the type before mapping connector args to fields. (hits: 1)
